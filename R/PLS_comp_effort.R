...
mw.clim <- mw.clim[mw.clim$state %in% "IN",] # for now, lets just run on indiana
...
# here characterize point level effort using maximum distance to tree:
trees.obj <- trees %>% group_by(x,y) %>% summarize(effort = max(rbind(dist1, dist2)), 
                                                   effort.area = (max(rbind(dist1, dist2))^2)*pi)
...
# clean up grid cells where we dont have full envtl data:
...
out <- readRDS("initial_GJAM_PLS_in_point_test.rds")
...
out.eff <- readRDS("initial_GJAM_PLS_in_point_test_effort.rds")



#----------------------using area as effort--------------------

xdata <-trees.spec.clim.eff[,c("total_pr", "ksat", "Mean_tm")]
plsYdata <- trees.spec.clim.eff[,6:38]
plsYdata <- plsYdata[!names(plsYdata) %in% "No tree"]
effort <- trees.spec.clim.eff$effort.area
rl   <- list(r = 4, N = 10)
# create a modelList object to feed into gjam()
# ng is number of gibbs samples, burnin = burn in period
# typenames is teh typ of data
...
# running the model with area based effort level at the point data:
effort.vals <- trees.spec.clim.eff$effort.area
effort <- list(columns = length(xdata), values = effort.vals)

# create model list with effort
ml   <- list(ng = 2500, burnin = 500, typeNames = 'DA', reductList = rl, effort = effort)
...
saveRDS(out.eff, "initial_GJAM_PLS_in_point_test_effort_area.rds")
